...
class ResultBloc extends Bloc<ResultEvent, ResultState> {
  ...
  FutureOr<void> fetchSessionResult(
    ...
    //try {
      String accessToken = await SharedPref.getString(SharedPreferenceKey().accessTokenKey);
      String schoolId = await SharedPref.getString(SharedPreferenceKey().schoolIdKey);
      ...
      if (resultRespomse.statusCode == 200 ||
          resultRespomse.statusCode == 201) {
        print(json.decode(resultRespomse.body));
        ResultModel resultModel = ResultModel.fromJson(jsonDecode(resultRespomse.body));
        emit(InitialSuccessState("Successful",resultModel));
      } else if (resultRespomse.statusCode == 401) {
        emit(AccessTokenExpireState());
      }else if(resultRespomse.statusCode == 500||resultRespomse.statusCode == 501){
        emit(ErrorState("There was a problem processing request please try again later."));
        emit(ResultInitial());
      } else {
